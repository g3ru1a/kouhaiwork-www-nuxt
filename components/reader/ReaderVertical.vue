	...
	<div class="w-full">
		...
		>
			<div class="mx-auto w-screen xl:w-auto xl:h-screen">
				<img class="w-full h-full" :src="mediaPage(page.media)" alt="" />
			</div>
		</div>
		<div
			id="vert-settings"
			class="flex fixed top-1/2 right-0 transform-all duration-150"
		>
			<button
				@click="toggleOpen(false)"
				class="bg-theme-dark text-white rounded-l-lg"
			...
			<div
				class="w-36 flex flex-col transition-all duration-150 flat"
				:class="{ '-mr-36': !open }"
			>
				<button
					v-if="next_id"
				...
				<p
					class="w-full py-2 bg-white border border-theme text-theme text-center dark:bg-dt-300 dark:border-dt-theme"
				>
					Ch {{ chapter.number }}
				</p>
				<button
...
export default {
	...
	methods: {
		carryVert(event) {
			...
			document.querySelector("#vert-settings").style.top =
				st + (window.innerHeight - 200) + "px";
